...
fwrite(valid[,c("FID","IID")], "geno_qc.valid.sample", sep="\t") 
...
pcs <- read.table("~/kcni-school-data/geno/geno_qc.eigenvec")  
...
```{bash,eval=F}
head ~/kcni-school-data/sumstats/PGC_UKB_depression_genome-wide.txt
```
...
```{r}
scores <- read.table("~/kcni-school-data/prs/MDD.prsice",header=T)
...
```{bash, eval = FALSE}
cat ~/kcni-school-data/prs/MDD_noregress.log
```  
...
prs <- read.table("~/kcni-school-data/prs/MDD_noregress.all_score",header=T)

pheno <- readRDS("~/kcni-school-data/pheno/phenotype_file.rds")
```  
