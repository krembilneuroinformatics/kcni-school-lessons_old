...
The data for today is located on figshare. We going to download and unzip it into our day1 directory. 

To find out where we are - it's good to use the R package here.
The R package `here` will help us all find the same "path" to our files. Let's use `here` to figure out the path to our day1 lessons directory.
...
In the bash terminal we can cd to this directory for the rest of the steps

```{sh, eval = FALSE}
## first we need to get inside the day 1 folder
## you can replace "day1" with the full path to day1 is you don't think you are in the correct place
cd day1
...
Now we should see a zip file appear in our directory - but we still need to unzip the file. Let's do that now.
...
awk 'NR>1{print exp($5)}' PGC_UKB_depression_genome-wide.txt | sed '1i OR' > or.txt
paste -d" " PGC_UKB_depression_genome-wide.txt or.txt > MDD_sumstats.txt
```  
...
Note: on some computers - this will get killed for RAM reasons if this happens to you - we will try to give you a precalculated version..

```{bash, eval = FALSE}
## cd ~/kcni-school-data/kcni-school-lessons/day1
